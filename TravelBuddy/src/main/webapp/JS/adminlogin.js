...
$(document).ready(function () {
    ...
    $(window).scroll(function () {
        if ($(this).scrollTop() > 50) {
            $('nav').addClass('bg-white shadow-lg').removeClass('bg-transparent');
            $('nav .btn').removeClass('text-white').addClass('text-primary');
        } else {
            $('nav').removeClass('bg-white shadow-lg').addClass('bg-transparent');
    ...
    // Admin login form animation
    $('#admin-login-form').on('submit', function (e) {
        e.preventDefault();
        const $btn = $(this).find('.admin-login-btn');
        $btn.removeClass('animate__pulse animate__infinite').addClass('animate__bounce');
        setTimeout(() => {
            this.submit(); // Proceed with form submission
        }, 500);
